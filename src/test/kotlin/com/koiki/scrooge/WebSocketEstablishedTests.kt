...
class WebSocketEstablishedTests(
    ...
    fun test() {
        ...
        val numberOfRecievedMessages: Long = 1

        val output: ReplayProcessor<String> = ReplayProcessor.create(numberOfRecievedMessages.toInt())

        StandardWebSocketClient().execute(URI(webSocketUri + scroogeId)) { session ->
            session
                    .toMono()
                    .thenMany(session.receive().take(numberOfRecievedMessages)
